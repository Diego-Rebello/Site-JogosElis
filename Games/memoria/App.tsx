...
import type { CardData, GameState, ModoDasCartas, Player } from './types';
...
const chaveRecorde = (cartas: number, modo: ModoDasCartas) => `jogos-elis:memoria:recorde:${cartas}${modo === 'emojis' ? '' : `:${modo}`}`;
function lerRecorde(cartas: number, modo: ModoDasCartas): Recorde | null {
  try { const valor = JSON.parse(localStorage.getItem(chaveRecorde(cartas, modo)) || 'null'); return valor && Number.isFinite(valor.jogadas) && Number.isFinite(valor.segundos) ? valor : null; } catch { return null; }
}
...
function salvarRecorde(cartas: number, modo: ModoDasCartas, recorde: Recorde) { try { localStorage.setItem(chaveRecorde(cartas, modo), JSON.stringify(recorde)); } catch { /* O jogo continua sem persistência. */ } }
const tempo = (segundos: number) => `${Math.floor(segundos / 60)}:${String(segundos % 60).padStart(2, '0')}`;

interface SetupProps { onStart: (jogadores: number, cartas: number, tema: NomeDoTema, modo: ModoDasCartas) => void }
const SetupScreen: React.FC<SetupProps> = ({ onStart }) => {
  const [jogadores, setJogadores] = useState(1); const [cartas, setCartas] = useState(cartasPadrao); const [tema, setTema] = useState<NomeDoTema>('Animais'); const [conteudo, setConteudo] = useState<'emojis' | 'contas'>('emojis'); const [tipoConta, setTipoConta] = useState<Exclude<ModoDasCartas, 'emojis'>>('mistas');
  ...
  return <main className="flex flex-1 items-center justify-center p-3 sm:p-5"><section className="w-full max-w-2xl rounded-2xl bg-white/75 p-5 text-center shadow-lg sm:p-8">
    ...
    <fieldset className="mb-5"><legend className="mb-3 text-xl font-bold text-gray-700">O que você quer combinar?</legend><div className="grid grid-cols-2 gap-2"><button className={seletor(conteudo === 'emojis')} aria-pressed={conteudo === 'emojis'} onClick={() => setConteudo('emojis')}>🐶 Emojis</button><button className={seletor(conteudo === 'contas')} aria-pressed={conteudo === 'contas'} onClick={() => setConteudo('contas')}>➕ Contas</button></div></fieldset>
    {conteudo === 'emojis'
      ...
      : <fieldset className="mb-5"><legend className="mb-3 text-xl font-bold text-gray-700">Escolha as contas</legend><div className="grid grid-cols-1 gap-2 sm:grid-cols-3">{([['soma','Somas'],['multiplicacao','Multiplicações'],['mistas','Misturadas']] as const).map(([id,nome]) => <button key={id} className={seletor(tipoConta === id)} aria-pressed={tipoConta === id} onClick={() => setTipoConta(id)}>{nome}</button>)}</div><p className="mt-3 text-gray-600">Combine cada conta com o resultado.</p></fieldset>}
    ...
    <button className="min-h-14 w-full rounded-2xl bg-emerald-700 px-5 text-2xl font-bold text-white shadow-lg hover:bg-emerald-800" onClick={() => onStart(jogadores, cartas, tema, conteudo === 'emojis' ? 'emojis' : tipoConta)}>Começar!</button>
  </section></main>;
};

interface CardProps { card: CardData; onClick: (id: number) => void; disabled: boolean }
const Card: React.FC<CardProps> = ({ card, onClick, disabled }) => <button type="button" data-face={card.face} data-par={card.chavePar} aria-label={card.isMatched ? `Par encontrado: ${card.face}` : card.isFlipped ? `Carta ${card.face}` : 'Carta virada'} disabled={disabled || card.isFlipped || card.isMatched} onClick={() => onClick(card.id)} className="aspect-[3/4] min-h-11 w-full [perspective:1000px] disabled:cursor-default">
  <span className={`relative block h-full w-full transition-transform duration-500 [transform-style:preserve-3d] ${card.isFlipped || card.isMatched ? '[transform:rotateY(180deg)]' : ''}`}>
    ...
    <span className={`absolute inset-0 flex items-center justify-center rounded-lg px-1 text-center font-bold shadow-md [backface-visibility:hidden] [transform:rotateY(180deg)] ${card.tipoFace === 'figura' ? 'text-3xl sm:text-4xl' : 'text-base text-gray-800 sm:text-2xl'} ${card.isMatched ? 'bg-emerald-200' : 'bg-white'}`}>{card.face}</span>
  </span>
</button>;
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (primeira && segunda && primeira.chavePar===segunda.chavePar) { tocar('acerto'); setAviso(`Par encontrado: ${primeira.face} combina com ${segunda.face}.`); setCombinadas(v=>[...v,a,b]); setPlayers(ps=>ps.map(p=>p.id===playerAtual?{...p,score:p.score+1}:p)); setViradas([]); setConferindo(false); return; }
  ...
  useEffect(() => {
    ...
    if(solo){const atual={jogadas:jogadasRef.current,segundos:segundosRef.current},anterior=lerRecorde(cardCount,modoDasCartas),novo=melhorQue(atual,anterior);if(novo)salvarRecorde(cardCount,modoDasCartas,atual);setNovoRecorde(novo);setRecorde(novo?atual:anterior)}
    setEstado('finished'); setAviso('Você encontrou todos os pares!'); tocar('vitoria'); lancarConfete();
  },[combinadas,cards,players.length,cardCount,modoDasCartas]);

  const iniciar = useCallback((quantidadeJogadores:number, quantidadeCartas:number, nomeTema:NomeDoTema, modo:ModoDasCartas) => { limparTimers();setCardCount(quantidadeCartas);setTema(nomeTema);setModoDasCartas(modo);setPlayers(Array.from({length:quantidadeJogadores},(_,i)=>({id:i+1,name:i===0?nomeCrianca:`Jogador ${i+1}`,score:0})));setCards(gerarCartas(quantidadeCartas,nomeTema,modo));setViradas([]);setCombinadas([]);setPlayerAtual(1);setConferindo(false);setTremendo(false);jogadasRef.current=0;segundosRef.current=0;setJogadas(0);setSegundos(0);setRecorde(quantidadeJogadores===1?lerRecorde(quantidadeCartas,modo):null);setNovoRecorde(false);registrada.current=false;setEstado('playing');tocar('clique') },[limparTimers]);
  const repetir = useCallback(() => iniciar(players.length,cardCount,tema,modoDasCartas),[iniciar,players.length,cardCount,tema,modoDasCartas]);
